...
CLIENT_ROOT=${LORKHAN_CLIENT_ROOT:?set LORKHAN_CLIENT_ROOT to the active LORKHAN worktree}
TMP=$(mktemp -d "${TMPDIR:-/tmp}/lorkhan-beast-http.XXXXXX")
PG_PORT=${LORKHAN_BEAST_PG_PORT:-55451}
HTTP_PORT=${LORKHAN_BEAST_HTTP_PORT:-58451}
BUILD_DIR=${LORKHAN_BEAST_BUILD_DIR:-$CLIENT_ROOT/build-beast}
PHP_PID=
WORKER_PID=

cleanup() {
    if [ "${LORKHAN_KEEP_TEST_TMP:-0}" = 1 ]; then printf 'test artifacts retained at %s\n' "$TMP" >&2; return; fi
...
createdb -h 127.0.0.1 -p "$PG_PORT" lorkhan_beast_http
...
LORKHAN_TEST_SERVER_STATE="$TMP/server-state" php "$ROOT/scripts/migrate.php" up >/dev/null
...
start_http
CLIENT_BIN=${LORKHAN_BEAST_CLIENT:-}
CONTROL_DIR_ARG="$TMP/control"
if [ -z "$CLIENT_BIN" ]; then
    cmake -S "$CLIENT_ROOT" -B "$BUILD_DIR" -DLORKHAN_WITH_BOOST_BEAST=ON >/dev/null
    cmake --build "$BUILD_DIR" --target lorkhan_beast_transport_tests --parallel >/dev/null
    CLIENT_BIN="$BUILD_DIR/components/lorkhan/lorkhan_beast_transport_tests"
