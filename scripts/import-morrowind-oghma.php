...
$usage=static function():never{fwrite(STDERR,"Usage:\n  php scripts/import-morrowind-oghma.php dry-run|apply --articles=PATH --manifest=PATH --catalog-version=VERSION\n  php scripts/import-morrowind-oghma.php rollback [--catalog-version=VERSION]\n  php scripts/import-morrowind-oghma.php status\n");exit(2);};
try{
    $command=$argv[1]??null;if(!in_array($command,['dry-run','apply','rollback','status'],true))$usage();$options=[];
    ...
    if(in_array($command,['dry-run','apply'],true)){foreach(['articles','manifest','catalog-version']as$required)if(!isset($options[$required]))$usage();
    ...
    elseif($command==='rollback')$result=$importer->rollback($options['catalog-version']??null);else$result=$importer->status();
