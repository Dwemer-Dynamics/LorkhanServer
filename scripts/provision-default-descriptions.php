...
$catalogDirectory = getenv('ALMSIVI_DESCRIPTION_CATALOG_DIR') ?: dirname(__DIR__) . '/resources/descriptions/morrowind-official';
...
if (!is_file($csv) && !is_file($manifest) && !is_file($versionFile)) {
    echo json_encode(['schema' => 'almsivi.default-descriptions.v1', 'status' => 'skipped', 'reason' => 'catalog_not_bundled'], JSON_THROW_ON_ERROR) . PHP_EOL;
    exit(0);
}
...
echo json_encode(['schema' => 'almsivi.default-descriptions.v1', 'status' => 'ready', 'result' => $result], JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES) . PHP_EOL;
