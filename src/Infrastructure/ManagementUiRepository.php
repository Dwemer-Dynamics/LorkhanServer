...
final class ManagementUiRepository
{
    ...
    public function rows(string $view): array
    {
        $sql = match ($view) {
            ...
            'characters', 'profiles', 'npc_biographies' => "SELECT p.profile_id,p.installation_id,p.name,p.current_revision,p.actor_identity,r.content,"
                ...
                . "FROM profiles p JOIN profile_revisions r ON r.profile_id=p.profile_id AND r.revision=p.current_revision WHERE p.deleted_at IS NULL AND COALESCE(p.actor_identity->>'kind','actor') NOT IN ('player','narrator') ORDER BY p.name LIMIT 500",
            ...
            'llm' => "SELECT c.configuration_id,c.installation_id,c.name,c.current_revision,r.content,"
                ...
                . "(SELECT count(*)::int FROM profiles p JOIN profile_revisions pr ON pr.profile_id=p.profile_id AND pr.revision=p.current_revision WHERE p.deleted_at IS NULL AND (pr.content->'routing'->>'llm_configuration_id'=c.configuration_id::text OR pr.content->'routing'->>'llm_fast_configuration_id'=c.configuration_id::text OR pr.content->'routing'->>'llm_powerful_configuration_id'=c.configuration_id::text OR pr.content->'routing'->>'llm_experimental_configuration_id'=c.configuration_id::text OR pr.content->'routing'->>'llm_fallback_configuration_id'=c.configuration_id::text)) AS profile_usage,r.created_at,"
            ...
            'prompts' => "SELECT c.configuration_id,c.installation_id,c.name,c.current_revision,r.content,(SELECT count(*)::int FROM profiles p JOIN profile_revisions pr ON pr.profile_id=p.profile_id AND pr.revision=p.current_revision WHERE p.deleted_at IS NULL AND pr.content->'routing'->>'prompt_configuration_id'=c.configuration_id::text) AS profile_usage,r.created_at,"
                . "(SELECT jsonb_agg(jsonb_build_object('revision',history.revision,'reason',history.change_reason,'created_at',history.created_at) ORDER BY history.revision DESC) FROM configuration_revisions history WHERE history.configuration_id=c.configuration_id) AS revisions "
            ...
            'action_policies' => "SELECT c.configuration_id,c.installation_id,c.name,c.current_revision,r.content,r.created_at,"
                . "(SELECT jsonb_agg(jsonb_build_object('revision',history.revision,'reason',history.change_reason,'created_at',history.created_at) ORDER BY history.revision DESC) FROM configuration_revisions history WHERE history.configuration_id=c.configuration_id) AS revisions "
                . "FROM configuration_sets c JOIN configuration_revisions r ON r.configuration_id=c.configuration_id AND r.revision=c.current_revision WHERE c.kind='action_policy' AND c.deleted_at IS NULL ORDER BY c.name LIMIT 100",
            ...
            'global_settings' => "SELECT installation_id,display_name,last_seen_at,created_at FROM installations WHERE revoked_at IS NULL ORDER BY last_seen_at DESC LIMIT 100",
