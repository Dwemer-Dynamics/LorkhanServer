...
final class ManagementUiRepository
{
    ...
    public function rows(string $view): array
    {
        $sql = match ($view) {
            ...
            'prompts' => "SELECT c.configuration_id,c.installation_id,c.name,COALESCE(p.prompt_key,c.name) AS prompt_key,c.current_revision,(r.content || jsonb_build_object('instruction',COALESCE(NULLIF(p.custom_prompt,''),p.default_prompt,r.content->>'instruction'),'default_prompt',p.default_prompt,'custom_prompt',p.custom_prompt,'description',COALESCE(p.description,r.content->>'description'))) AS content,((SELECT count(*) FROM profiles prf JOIN profile_revisions pr ON pr.profile_id=prf.profile_id AND pr.revision=prf.current_revision WHERE prf.deleted_at IS NULL AND pr.content->'routing'->>'prompt_configuration_id'=c.configuration_id::text)+(SELECT count(*) FROM core_profiles cp JOIN core_profile_revisions cr ON cr.core_profile_id=cp.core_profile_id AND cr.revision=cp.current_revision WHERE cp.deleted_at IS NULL AND cr.content->'routing'->>'prompt_configuration_id'=c.configuration_id::text))::int AS profile_usage,r.created_at,"
                . "(SELECT jsonb_agg(jsonb_build_object('revision',history.revision,'reason',history.change_reason,'created_at',history.created_at) ORDER BY history.revision DESC) FROM configuration_revisions history WHERE history.configuration_id=c.configuration_id) AS revisions "
                . "FROM configuration_sets c JOIN configuration_revisions r ON r.configuration_id=c.configuration_id AND r.revision=c.current_revision LEFT JOIN prompts p ON p.installation_id=c.installation_id AND p.source_configuration_id=c.configuration_id WHERE c.kind='prompt' AND c.deleted_at IS NULL ORDER BY c.name LIMIT 100",
            ...
            'response_queue' => "SELECT COALESCE(s.speaker,'Unknown') AS speaker,left(s.speech,1000) AS text,s.delivery_state,"
                . "CASE WHEN s.delivery_state IN ('emitted','pending') AND d.delivery_deadline_at<=clock_timestamp() THEN 'overdue' ELSE 'current' END AS queue_health,"
                . "s.created_at AS emitted_at,d.delivery_deadline_at,d.delivered_at FROM speech s LEFT JOIN dialogue_utterances d ON d.dialogue_message_id=s.dialogue_message_id ORDER BY s.rowid DESC LIMIT 100",
