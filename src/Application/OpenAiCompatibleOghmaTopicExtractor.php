...
final class OpenAiCompatibleOghmaTopicExtractor implements OghmaTopicExtractor
{
        ...
        private readonly bool $disableReasoning=false)
    {
        ...
        if($model===''||strlen($model)>200||$timeoutMs<250||$timeoutMs>30_000)throw new \InvalidArgumentException('invalid_oghma_extractor_configuration');
    }
    ...
    public function extract(string $context,int $limit,CancellationToken $cancellation):array
    {
        ...
        $request=['model'=>$this->model,'temperature'=>0,'response_format'=>['type'=>'json_object'],'messages'=>[
