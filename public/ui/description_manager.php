...
function almsivi_description_url(array $changes = []): string
{
...
<main class="descriptions-page">
    <header class="page-header almsivi-page-head"><h1>Description Manager</h1><p class="page-subtitle almsivi-page-head-note">Create custom descriptions for Morrowind items and equipment that enhance NPC context</p></header>
    ...
    <form class="installation-selector" method="get" action="<?php echo almsivi_ui_h($webRoot . '/ui/description_manager.php'); ?>">
        ...
        <label>Installation<select name="installation_id" data-description-installation><?php foreach ($installations as $id => $label): ?><option value="<?php echo almsivi_ui_h($id); ?>"<?php echo $id === $installationId ? ' selected' : ''; ?>><?php echo almsivi_ui_h($label); ?></option><?php endforeach; ?></select></label>
    ...
    <div class="content-grid">
        <section class="content-section">
            <h2>Batch Upload</h2>
            <form action="<?php echo almsivi_ui_h($managementBasePath . '/forms/description-import'); ?>" method="post" enctype="multipart/form-data">
                ...
                <div class="button-group"><button type="submit" class="action-button upload-csv">Upload CSV</button><a class="action-button download-csv" href="<?php echo almsivi_ui_h($managementBasePath . '/exports/descriptions/example.csv'); ?>">Download Example CSV</a><a class="action-button export-csv" href="<?php echo almsivi_ui_h($managementBasePath . '/exports/descriptions/custom.csv?' . http_build_query(['installation_id'=>$installationId])); ?>">Export Custom Descriptions</a></div>
            </form>
        ...
        <section class="content-section">
            ...
            <form method="post" action="<?php echo almsivi_ui_h($managementBasePath . '/forms/description-reset'); ?>" data-confirm="Delete every custom description for this installation?">
                <input type="hidden" name="_csrf" value="<?php echo almsivi_ui_h($csrf); ?>"><input type="hidden" name="installation_id" value="<?php echo almsivi_ui_h($installationId); ?>"><input type="hidden" name="confirm" value="Reset"><button type="submit" class="btn-danger">Factory Reset Item Override Table</button>
            </form>
        </section>
    </div>
    <section class="full-width-section discovered-section">
        ...
        <div class="table-container"><table><thead><tr><th>Base ID</th><th>Plugin</th><th>Name</th><th>Seen In</th><th>Status</th></tr></thead><tbody><?php foreach ($discoveredItems as $item): ?><tr><td><?php echo almsivi_ui_h($item['record_id']); ?></td><td><?php echo almsivi_ui_h($item['content_file']); ?></td><td><?php echo almsivi_ui_h($item['display_name'] ?? ''); ?></td><td><?php $sources=$item['observed_sources']??[];if(is_string($sources))$sources=json_decode($sources,true);echo almsivi_ui_h(is_array($sources)?implode(', ',$sources):''); ?></td><td><span class="source-badge source-<?php echo almsivi_ui_h(strtolower((string)$item['description_source'])); ?>"><?php echo almsivi_ui_h($item['description_source']); ?></span><?php if (!filter_var($item['active'] ?? false,FILTER_VALIDATE_BOOL)): ?> <span class="source-badge inactive">Inactive Plugin</span><?php endif; ?></td></tr><?php endforeach; ?><?php if ($discoveredItems === []): ?><tr><td colspan="5">No items have been discovered in game yet.</td></tr><?php endif; ?></tbody></table></div>
    </section>
    <section class="full-width-section">
        ...
        <div class="action-container"><button type="button" class="action-button add-new" data-description-create>Add New Entry</button>
            <form class="search-container" method="get" action="<?php echo almsivi_ui_h($webRoot . '/ui/description_manager.php'); ?>"><input type="hidden" name="installation_id" value="<?php echo almsivi_ui_h($installationId); ?>"><?php if ($embedded): ?><input type="hidden" name="embed" value="1"><?php endif; ?><input type="hidden" name="letter" value="<?php echo almsivi_ui_h($filters['letter']); ?>"><input type="hidden" name="source" value="<?php echo almsivi_ui_h($filters['source']); ?>"><input type="hidden" name="plugin" value="<?php echo almsivi_ui_h($filters['plugin']); ?>"><input type="search" name="search" value="<?php echo almsivi_ui_h($filters['search']); ?>" placeholder="Search descriptions..." aria-label="Search descriptions"><button type="submit" class="action-button edit">Search</button></form>
        </div>
        ...
        <div class="filter-section"><strong>Filter by Name:</strong><div class="filter-buttons"><a class="alphabet-button<?php echo $filters['letter']===''?' active':''; ?>" href="<?php echo almsivi_ui_h(almsivi_description_url(['letter'=>'','page'=>1])); ?>#entries">All</a><?php foreach (range('A','Z') as $letter): ?><a class="alphabet-button<?php echo $filters['letter']===$letter?' active':''; ?>" href="<?php echo almsivi_ui_h(almsivi_description_url(['letter'=>$letter,'page'=>1])); ?>#entries"><?php echo $letter; ?></a><?php endforeach; ?></div></div>
        <section class="new-entry-panel" data-description-panel hidden><h2>Add New Entry</h2><form method="post" action="<?php echo almsivi_ui_h($managementBasePath . '/forms/description-save'); ?>"><input type="hidden" name="_csrf" value="<?php echo almsivi_ui_h($csrf); ?>"><input type="hidden" name="installation_id" value="<?php echo almsivi_ui_h($installationId); ?>"><div class="entry-form-grid"><label>Content file<input name="content_file" value="Morrowind.esm" maxlength="256" required></label><label>Record ID<input name="record_id" maxlength="256" required></label><label>Display name<input name="display_name" maxlength="256" required></label><label class="wide">Description<textarea name="description" maxlength="8192" required></textarea></label></div><div class="button-group"><button class="action-button add-new" type="submit">Save description</button><button type="button" class="action-button" data-description-cancel>Cancel</button></div></form></section>
        <p class="catalog-result-count">Showing <?php echo count($catalog['items']); ?> of <?php echo (int)$catalog['total']; ?> matching descriptions.</p>
        <div class="table-container"><table><thead><tr><th>Base ID</th><th>Plugin</th><th>Name</th><th>Description</th><th>Source</th><th>Actions</th></tr></thead><tbody><?php foreach ($catalog['items'] as $row): $isCustom=$row['source']==='custom'; ?><tr><td><?php echo almsivi_ui_h($row['record_id']); ?></td><td><?php echo almsivi_ui_h($row['content_file']); ?></td><td><?php echo almsivi_ui_h($row['display_name']); ?></td><td><?php echo almsivi_ui_h($row['description'] ?? 'No description has been provided.'); ?></td><td><span class="source-badge source-<?php echo almsivi_ui_h($row['source']); ?>"><?php echo almsivi_ui_h(ucfirst((string)$row['source'])); ?></span><?php if ($row['active'] !== null && !filter_var($row['active'],FILTER_VALIDATE_BOOL)): ?> <span class="source-badge inactive">Inactive Plugin</span><?php endif; ?></td><td><details><summary><?php echo $isCustom?'Edit':'Add Override'; ?></summary><form method="post" action="<?php echo almsivi_ui_h($managementBasePath . '/forms/description-save'); ?>"><input type="hidden" name="_csrf" value="<?php echo almsivi_ui_h($csrf); ?>"><input type="hidden" name="installation_id" value="<?php echo almsivi_ui_h($installationId); ?>"><input name="content_file" value="<?php echo almsivi_ui_h($row['content_file']); ?>" required><input name="record_id" value="<?php echo almsivi_ui_h($row['record_id']); ?>" required><input name="display_name" value="<?php echo almsivi_ui_h($row['display_name']); ?>" required><textarea name="description" required><?php echo almsivi_ui_h($row['description'] ?? ''); ?></textarea><button type="submit">Save Override</button></form></details><?php if ($isCustom): ?><form method="post" action="<?php echo almsivi_ui_h($managementBasePath . '/forms/description-delete'); ?>" data-confirm="Delete this custom override?"><input type="hidden" name="_csrf" value="<?php echo almsivi_ui_h($csrf); ?>"><input type="hidden" name="installation_id" value="<?php echo almsivi_ui_h($installationId); ?>"><input type="hidden" name="description_id" value="<?php echo almsivi_ui_h($row['description_id']); ?>"><button type="submit" class="btn-danger">Delete</button></form><?php endif; ?></td></tr><?php endforeach; ?><?php if ($catalog['items'] === []): ?><tr><td colspan="6">No descriptions match these filters.</td></tr><?php endif; ?></tbody></table></div>
        <?php if ((int)$catalog['pages'] > 1): ?><nav class="description-pagination" aria-label="Description pages"><?php if ((int)$catalog['page'] > 1): ?><a href="<?php echo almsivi_ui_h(almsivi_description_url(['page'=>(int)$catalog['page']-1])); ?>#entries">Previous</a><?php endif; ?><span>Page <?php echo (int)$catalog['page']; ?> of <?php echo (int)$catalog['pages']; ?></span><?php if ((int)$catalog['page'] < (int)$catalog['pages']): ?><a href="<?php echo almsivi_ui_h(almsivi_description_url(['page'=>(int)$catalog['page']+1])); ?>#entries">Next</a><?php endif; ?></nav><?php endif; ?>
    </section>
    <?php endif; ?>
</main>
