...
function almsivi_ui_feature_registry(): array
{
    static $features = [
        ...
        'config.llm.generation' => ['title' => 'Generation Controls', 'state' => 'live', 'description' => 'Timeout, token limits, temperature, streaming, JSON mode, and disable-reasoning can be set per connector. Blank sampling fields use provider defaults for direct connectors or inherit server settings for configured connectors.'],
        ...
        'config.llm.reasoning-fix' => ['title' => 'Reasoning Model Fix', 'state' => 'planned', 'description' => 'Herika strips reasoning tags out of a response before parsing it. ALMSIVI does not do that cleanup. The separate Disable reasoning override only asks the provider to skip reasoning output and performs no tag cleaning.'],
