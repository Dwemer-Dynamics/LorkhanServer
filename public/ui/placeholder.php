...
<main class="container-fluid feature-placeholder-page">
    <section class="page-header">
        <h1 class="api-title"><?php echo lorkhan_ui_h($feature['title']); ?></h1>
        <p class="page-subtitle"><?php echo lorkhan_ui_h($feature['description']); ?></p>
    </section>
    <section class="feature-placeholder-panel" aria-labelledby="feature-placeholder-heading">
        <div class="feature-placeholder-heading">
            <h2 id="feature-placeholder-heading"><?php echo lorkhan_ui_h($feature['title']); ?></h2>
            <?php echo lorkhan_ui_feature_badge($featureId); ?>
        </div>
        <p>The HerikaServer presentation is retained so this control remains in its expected location. It performs no action in LORKHAN.</p>
        <?php if ($feature['controls'] !== []): ?>
        <div class="feature-placeholder-controls">
            <?php foreach ($feature['controls'] as $control): echo lorkhan_ui_placeholder_control((string) $control, $featureId); endforeach; ?>
        </div>
        <?php endif; ?>
    </section>
</main>
