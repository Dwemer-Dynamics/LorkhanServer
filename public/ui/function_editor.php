...
$policyFields = static function (array $content, array $actions): void {
    ...
    $selected = count(array_filter($actions, $isAllowed));
    ?>
    <div class="policy-form-grid">
        ...
        <fieldset class="allowed-actions" data-action-policy-controls><legend>Allowed OpenMW actions</legend>
            <div class="action-policy-bulk" data-action-bulk data-action-bulk-class="action-button"><output class="action-policy-count" data-action-selected-count><?php echo $selected; ?> of <?php echo count($actions); ?> selected</output></div>
            <div class="action-policy-grid"><?php foreach ($actions as $action): $name = (string) $action['action_name']; ?><label><input type="checkbox" name="allowed_actions[]" value="<?php echo almsivi_ui_h($name); ?>"<?php echo $isAllowed($action) ? ' checked' : ''; ?>> <strong><?php echo almsivi_ui_h($name); ?></strong><small>Tier <?php echo almsivi_ui_h($action['tier']); ?> &mdash; <?php echo almsivi_ui_h($action['description']); ?></small></label><?php endforeach; ?></div>
        </fieldset>
    </div>
<?php };
?>
<main class="action-editor-page">
    ...
    <header class="page-header almsivi-page-head"><h1 class="almsivi-page-head-title">Action Editor</h1><p class="almsivi-page-head-note">Review the negotiated OpenMW action catalogue and manage scoped action policies.</p></header>
    ...
    <section class="how-section"><h2>How It Works</h2><p>The catalogue below is read-only. Use an <strong>Action Policy</strong> to choose which actions an installation or NPC profile may use, and the highest tier allowed.</p><p class="replacement-note">ALMSIVI keeps the negotiated OpenMW action catalogue immutable. Use scoped Action Policies to enable actions by installation or NPC profile. <?php echo almsivi_ui_feature_badge('config.actions.direct-edit', true); ?></p></section>
