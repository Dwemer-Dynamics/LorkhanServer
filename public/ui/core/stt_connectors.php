 ...
 <div class="page-shell">
  ...
  <div class="layout">
   ...
   <section class="right-col">
    ...
    <div class="btn-row"><button class="btn-save" type="submit" form="stt-form">Save</button><form method="post" action="<?php echo lorkhan_ui_h($managementBasePath); ?>/forms/connector-test"><input type="hidden" name="_csrf" value="<?php echo lorkhan_ui_h($csrf); ?>"><input type="hidden" name="installation_id" value="<?php echo lorkhan_ui_h($installationId); ?>"><input type="hidden" name="kind" value="stt_provider"><input type="hidden" name="configuration_id" value="<?php echo lorkhan_ui_h($selected['configuration_id']); ?>"><button class="btn-primary" type="submit">Test</button></form><span class="excluded-control"><button class="btn-secondary" type="button" disabled aria-disabled="true" title="Browser dictation cannot preserve the in-game target/session fence.">Google Free STT</button><?php echo lorkhan_ui_feature_badge('config.stt.google-free',true); ?></span></div>
    ...
    <form id="stt-form" method="post" action="<?php echo lorkhan_ui_h($managementBasePath); ?>/forms/connector-revise">
     ...
     <div class="meta-group active"><h3><?php echo lorkhan_ui_h($definitions[$activeDriver]['label']); ?> Settings</h3><div class="inline-two">
      ...
      <?php foreach($optionCatalog[$activeDriver] as $field): $name=(string)$field['name'];$value=$options[$name]??''; ?><div class="field-block"><label><?php echo lorkhan_ui_h($field['label']); ?></label><?php if($field['type']==='boolean'): ?><label class="boolean-field"><input name="option__<?php echo lorkhan_ui_h($name); ?>" type="checkbox" value="1"<?php echo $value===true?' checked':''; ?>> Enabled</label><?php elseif($field['type']==='select'): ?><select name="option__<?php echo lorkhan_ui_h($name); ?>"><?php foreach($field['values'] as $choice): ?><option value="<?php echo lorkhan_ui_h($choice); ?>"<?php echo (string)$value===(string)$choice?' selected':''; ?>><?php echo lorkhan_ui_h($choice); ?></option><?php endforeach; ?></select><?php endif; ?></div><?php endforeach; ?>
     </div><div class="field-block advanced-json"><label for="stt-options">Advanced connector options (JSON)</label><textarea id="stt-options" name="options_json"><?php echo lorkhan_ui_h(json_encode($options===[]?(object)[]:$options,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES)); ?></textarea></div></div>
    </form>
    <?php endif; ?>
   </section>
  </div><?php endif; ?>
