...
$npcDisabledBool=static function(string$label,mixed$value,string$featureId):void{$display=$value===true?'Enabled':($value===false?'Disabled':'Inherit'); ?>
    <label class="feature-placeholder-card"><?php echo lorkhan_ui_h($label); ?> <?php echo lorkhan_ui_feature_badge($featureId,true); ?><select disabled aria-disabled="true"><option><?php echo lorkhan_ui_h($display); ?></option></select></label>
<?php };
$npcDisabledNumber=static function(string$section,string$field,string$label,string$featureId)use($overrides):void{$value=$overrides[$section][$field]??''; ?>
    <label class="feature-placeholder-card"><?php echo lorkhan_ui_h($label); ?> <?php echo lorkhan_ui_feature_badge($featureId,true); ?><input type="text" value="<?php echo lorkhan_ui_h($value===''?'Inherit':$value); ?>" disabled aria-disabled="true"></label>
<?php };
?>
...
<fieldset class="lorkhan-group"><legend>NPC connector overrides</legend><div class="lorkhan-form-grid">
    <?php $npcRoute('prompt_configuration_id','Dialogue prompt',$prompts);$npcRoute('llm_configuration_id','Standard LLM',$llm);$npcRoute('llm_fast_configuration_id','Fast LLM',$llm);$npcRoute('llm_powerful_configuration_id','Powerful LLM',$llm);$npcRoute('llm_experimental_configuration_id','Experimental LLM',$llm);$npcRoute('llm_fallback_configuration_id','Fallback LLM',$llm);$npcRoute('oghma_configuration_id','Oghma extractor',$llm);$npcRoute('tts_configuration_id','TTS connector',$tts); ?>
    <label>Diary LLM <?php echo lorkhan_ui_feature_badge('config.profiles.diary-llm',true); ?><select disabled aria-disabled="true"><option>Inherited from Core Profile</option></select></label>
    <label>Formatter LLM <?php echo lorkhan_ui_feature_badge('config.profiles.formatter-llm',true); ?><select disabled aria-disabled="true"><option>Not configured</option></select></label>
