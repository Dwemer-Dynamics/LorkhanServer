...
function almsivi_ui_npc_editor_form(array $row,array $voiceOptions,array $promptRows,array $llmRows,array $ttsRows,array $coreProfileRows,string $managementBasePath,string $csrf,bool$creating=false,array$installationOptions=[],array$effectiveSettings=[]):void
{
    ...
    $field('relationship_configuration_id','Relationship LLM','select',$relationshipRoute,$relationshipOptions,'span-2','Inherit uses the Core Profile connector. Disabled stops evaluation for this NPC. Default chance 0 makes no calls; 100 evaluates every eligible played response. Saved relationships still appear in prompts.');
    ...
    $field('setting_relationship_locked','Relationship Lock','select',$relationshipLock===true?'1':($relationshipLock===false?'0':'inherit'),['inherit'=>'Inherit Core Profile','1'=>'Locked','0'=>'Unlocked'],'','Stop relationship evaluation. Separate from the profile lock on the General tab.');
    echo'<div class="npc-editor-placeholder span-2"><h3>Relationship Builder '.almsivi_ui_feature_badge('config.npc.relationship-builder',true).'</h3><p>'.almsivi_ui_h(almsivi_ui_feature('config.npc.relationship-builder')['description']).'</p><button type="button" class="btn-base" disabled aria-disabled="true">Build Relationships</button></div></section>';
