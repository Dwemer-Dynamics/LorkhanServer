...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM almsivi_internal.configuration_sets WHERE kind='translation_policy')
        OR EXISTS (SELECT 1 FROM almsivi_internal.provider_attempts WHERE provider_kind='translation') THEN
...
DROP INDEX almsivi_internal.one_translation_policy_per_installation;
