...
WITH missing AS (
    SELECT i.installation_id, md5('almsivi:global-stt:' || i.installation_id::text)::uuid AS configuration_id
    FROM almsivi_internal.installations i
    WHERE NOT EXISTS (
        SELECT 1 FROM almsivi_internal.configuration_sets c
        WHERE c.installation_id=i.installation_id AND c.kind='stt_provider' AND c.deleted_at IS NULL
    )
), inserted_sets AS (
    INSERT INTO almsivi_internal.configuration_sets(configuration_id,installation_id,profile_id,kind,name,current_revision)
    ...
    INSERT INTO almsivi_internal.configuration_revisions(configuration_id,revision,content,change_reason)
...
INSERT INTO almsivi_internal.installation_provider_selections(installation_id,provider_kind,configuration_id)
SELECT c.installation_id,'stt_provider',c.configuration_id
FROM almsivi_internal.configuration_sets c
JOIN inserted_revisions r ON r.configuration_id=c.configuration_id
WHERE NOT EXISTS (
        SELECT 1 FROM almsivi_internal.installation_provider_selections s
...
WHERE c.kind='stt_provider' AND c.deleted_at IS NULL
  AND NOT EXISTS (SELECT 1 FROM almsivi_internal.installation_provider_selections s
      WHERE s.installation_id=c.installation_id AND s.provider_kind='stt_provider')
