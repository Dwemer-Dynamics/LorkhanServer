...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM almsivi_internal.memory_embeddings)
        OR EXISTS (SELECT 1 FROM almsivi_internal.configuration_sets WHERE kind='memory_embedding_policy')
        OR EXISTS (SELECT 1 FROM almsivi_internal.durable_jobs WHERE job_type='memory.embed') THEN
        RAISE EXCEPTION 'Cannot remove semantic memory support while embeddings, policy history or jobs exist.';
    END IF;
END $$;

DROP INDEX almsivi_internal.memory_embeddings_policy_revision;
DROP TABLE almsivi_internal.memory_embeddings;
...
DROP INDEX almsivi_internal.one_memory_embedding_policy_per_installation;
