...
CREATE TABLE lorkhan_internal.content_manifests (
    ...
    source_session_id uuid REFERENCES lorkhan_internal.sessions(session_id) ON DELETE SET NULL,
    source_turn_id uuid REFERENCES lorkhan_internal.turns(turn_id) ON DELETE SET NULL,
...
CREATE UNIQUE INDEX content_manifest_files_active_order_uq
    ON lorkhan_internal.content_manifest_files(installation_id,load_order) WHERE active;
CREATE INDEX content_manifest_files_active_name_idx
    ON lorkhan_internal.content_manifest_files(installation_id,lower(content_file)) WHERE active;
...
CREATE INDEX discovered_items_recent_idx ON lorkhan_internal.discovered_items(installation_id,last_seen_at DESC);

CREATE FUNCTION lorkhan_internal.observe_openmw_item(source_installation uuid,source_session uuid,source_turn uuid,
    source_time timestamptz,source_name text,item jsonb)
RETURNS void LANGUAGE plpgsql SET search_path=lorkhan_internal,public,pg_temp AS $function$
DECLARE
...
CREATE FUNCTION lorkhan_internal.sync_openmw_record_identity()
RETURNS trigger LANGUAGE plpgsql SET search_path=lorkhan_internal,public,pg_temp AS $function$
DECLARE
    session_row record;
    file_name text;
...
AFTER INSERT ON lorkhan_internal.turns FOR EACH ROW EXECUTE FUNCTION lorkhan_internal.sync_openmw_record_identity();
