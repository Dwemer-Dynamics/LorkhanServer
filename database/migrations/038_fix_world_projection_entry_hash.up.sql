-- Disambiguate the journal-entry digest from the projected metadata column.
DO $fix_world_projection$
DECLARE
    definition text;
BEGIN
    ...
    IF position('DECLARE computed_entry_hash char(64);' IN definition) > 0 THEN
        RETURN;
    END IF;
    IF position('DECLARE entry_hash char(64);' IN definition) = 0
        OR position('entry_hash := encode(sha256(convert_to(item::text,''UTF8'')),''hex'');' IN definition) = 0
        OR position('metadata.entry_hash=entry_hash;' IN definition) = 0
        OR position('source_row.session_id,record_key,source_turn,entry_hash' IN definition) = 0
    THEN
