...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM lorkhan_internal.memory_model_summaries)
        OR EXISTS (SELECT 1 FROM lorkhan_internal.configuration_sets WHERE kind='memory_policy') THEN
        RAISE EXCEPTION 'Cannot remove model memory support while summaries or policy history exist.';
    END IF;
END $$;
DROP TABLE lorkhan_internal.memory_model_summaries;
DROP INDEX lorkhan_internal.one_memory_policy_per_installation;
