...
CREATE TABLE lorkhan_internal.memory_embeddings (
    ...
    FOREIGN KEY(memory_id,memory_revision)
        REFERENCES lorkhan_internal.memory_record_revisions(memory_id,revision) ON DELETE CASCADE,
    FOREIGN KEY(policy_configuration_id,policy_revision)
        REFERENCES lorkhan_internal.configuration_revisions(configuration_id,revision)
);

CREATE INDEX memory_embeddings_policy_revision
    ON lorkhan_internal.memory_embeddings(policy_configuration_id,policy_revision,memory_id);
