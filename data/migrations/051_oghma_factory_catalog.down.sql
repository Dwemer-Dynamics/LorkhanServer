DELETE FROM lorkhan_internal.knowledge_documents AS document
USING lorkhan_internal.oghma_factory_documents AS factory
WHERE document.document_id=factory.document_id;
DROP TABLE IF EXISTS lorkhan_internal.oghma_factory_documents;
DROP TABLE IF EXISTS lorkhan_internal.oghma_catalog_entries;
DROP TABLE IF EXISTS lorkhan_internal.oghma_catalogs;
DROP TABLE IF EXISTS lorkhan_internal.oghma_installation_settings;
DROP INDEX IF EXISTS lorkhan_internal.knowledge_factory_topic_uq;
...
BEGIN
    ...
    IF projected_topic IS NULL THEN
        ...
        VALUES(projected_topic,NEW.content,to_tsvector('simple',NEW.content),'Morrowind',left(NEW.content,1000),
            'Morrowind',array_to_string(NEW.lexical_terms,','),COALESCE(NEW.provenance->>'category','LORKHAN'),'');
    ...
    ELSE
        UPDATE oghma SET topic_desc=NEW.content,native_vector=to_tsvector('simple',NEW.content),
            ...
            tags=array_to_string(NEW.lexical_terms,','),category=COALESCE(NEW.provenance->>'category','LORKHAN')
