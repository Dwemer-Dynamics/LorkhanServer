...
| `playthrough_manager.php` | Same path | Selected-scope overview, paired export/import panels and installation selection are implemented. Access beyond 100 rows, out-of-page selected-ID lookup and later-page empty states already have implementation and isolated HTTP proof. Remaining: full stored snapshots, create/switch/delete, automatic rollback snapshots and snapshot timeline. Profile-scoped JSON exports are not substitutes for full database snapshots. |
