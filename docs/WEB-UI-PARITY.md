...
| `playthrough_manager.php` | Same path | Full named database snapshots now support save, copy, download, delete, automatic rollback and live-source provenance. Active Database and paired Save/Stored panels use the reference structure; legacy profile-only tools are separately collapsed. Restricted-role lifecycle and populated/empty desktop/narrow checks passed. Remaining: snapshot timeline and recorded game-time metadata. Profile-scoped JSON exports are not substitutes for full database snapshots. |
