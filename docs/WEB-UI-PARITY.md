...
## Core editor structure and chained actions (2026-09-12)

Fresh live reference/native comparison finds the same 17 main section titles in
the same order after moving the native Revision Note out of its extra primary
card into Advanced Metadata. The submitted change_reason field and its default
are preserved. Edited notes mark the form dirty; a blank required note opens the
collapsed disclosure and receives focus. Correcting it restores form validity.

At 1280px the layout columns, gap and editor-card padding match (340px sidebar,
16px gap, 16px card padding). At 390px the live reference keeps a 260px sidebar
and squeezes its editor; Lorkhan retains its existing usable stacked layout.
This narrow-screen difference is deliberate and is not a claim of identical
geometry. No colors, server policies, provider configuration or game state changed.

The editor interaction pass also reproduced a stale revision between Copy to all
and named presets: after two successful copies the preset submitted revision 3
instead of the returned revision 5. Preset requests now use the current shared
form revision, falling back to the initial toolbar revision where necessary.
The same browser sequence now passes at both widths; simulated genuine conflicts
retain the unrelated name and word-limit drafts. No user profile was changed.

core-controls-review.cjs checks all 15 enabled keyboard toggles and their On/Off
labels, 10 slider/number pairs in both directions, and cancel/typed-confirm for
all 15 Copy to all dialogs at 1280/390. The protected default-profile checkbox is
correctly disabled and excluded from toggle attempts. core-copy-preset-review.cjs
checks repeated copy then preset application; core-audit-note-review.cjs checks
collapsed/default/edited/invalid/corrected note states. All writes are intercepted,
with no paid provider calls and no browser errors. Note screenshots inspected.

1048 server checks, 111 protocol files, PHP and JS syntax and diff whitespace
checks pass. All 858 deployed files match, with no extra/old paths and private
files still forbidden. Scoped code rollback:
...
## NPC evolution report (2026-09-12)

Profile Versions now links to the exact NPC's report in a new tab. The report
page copies the reference title/card/Markdown presentation with gold accents.
A deliberate Generate action queues work; simply opening history or the report
page never sends a provider request. Queued, generating, succeeded, failed,
disabled-connector and uncertain-response states are explicit. Retries retain a
request UUID, and reloading a confirmed job only checks its status. Markdown is
rendered with text/bold/bullet DOM nodes, never model-supplied HTML.

Native reports use the saved enabled Summaries connector, the existing native
memory-task route corresponding to Herika's Background & Memory Tasks. Broader
global connector composition/naming parity remains open. The report freezes the
provider revision and deduplicated chronological personality/backstory snapshots
for the exact installation/profile UUID. Empty or oversized history is rejected
explicitly instead of silently truncated. Up to 400 distinct snapshots/80KB of
encoded entries are accepted; report output is bounded to 8192 UTF-8 bytes. The
provider request uses the reference 4096-token report allowance. A single-attempt
job prevents hidden paid retries. Report output lives in its own table, so locked
NPCs can be analyzed and later profile edits cannot be overwritten by a report.

Migration 103 adds the report table; revert refuses to drop saved reports.
Schema inventory now has 177 relations and 1626 columns, hash
c772b6ac9e49d0b32a25145e4af84d74ae5912f9f56fc8596c44a665473cad1a.
Focused existing integration coverage passed against an isolated migrated DB:
disabled routing, history dedup/order, request idempotency, frozen history after
an NPC edit, mock worker result, lease fencing, scoped access and no NPC revision.
HTTP page/status/CSRF checks also pass in npc-report-probe.txt. The real provider
adapter passes JSON schema/prefill/4096-token/output checks against a localhost
synthetic responder in npc-report-transport.py. 1098 server checks, 111 protocol
files, PHP/JS lint and diff checks pass. The full integration/HTTP suites were not
rerun for this feature; their changed paths were exercised by these focused probes.

Deployed browser checks at 1280/390 pass report navigation, no automatic generation,
queued/success, safe Markdown, disabled routing, lost-response retry identity and
dead-job handling; all writes mocked. Report and version-viewer screenshots were
inspected, including corrected gold controls and wrapped header actions. Local
rollback code: /var/backups/lorkhanserver-code.W3Xef1. The new table and registered
worker type are present; the worker allows profile.report and the live report
table remains empty (no real requests made). Configuration, credentials and voice
contents are preserved. Full webpage parity is still active.

## Separate Background & Memory Tasks routing (2026-09-12)

Correction to the report checkpoint above: Herika uses CORE_CONNECTOR_MEDIUMTERM
for Background & Memory Tasks, separately from CORE_CONNECTOR_SUMMARY. Reports
now use the distinct global Background & Memory Tasks selector, not Summaries.
Older saved settings default this new route to Disabled; no provider is silently
assigned. The selector participates in Global Settings save, presets, connector
tests and provider usage/deletion protection. Queued reports retain their frozen
connector revision and protect it from deletion even after the selector clears.
Clearing the route also prevents pending report execution.

1100 server checks and 111 protocol files pass. The focused isolated report
probe passes with Summaries disabled and a separate background connector,
including frozen routing and selected/queued connector deletion guards.
Actual browser form submissions at 1280/390 save, clear, restore and reload the
new selector; narrow rendering was inspected with no horizontal overflow.
Evidence: background-route-probe.txt and background-route-review.cjs in local
Temp. No game or paid provider was invoked. Broader background-memory runtime
semantics and other unfinished matrix items remain open.

Local deployment completed with rollback /var/backups/lorkhanserver-code.grmKts.
All 875 runtime files match source, with no extra files or old paths; private
