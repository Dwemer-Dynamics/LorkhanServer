...
| Providers | Vetted final Synthserver LLM/STT/TTS set, mock mode, health, timeout/cancel/redaction, secret-free portable presets, dynamically labelled runtime-supported provider controls at connector creation and edit time with bounded advanced JSON compatibility, connector-level male/female fallback voices, bounded single-WAV or ZIP-batch voice sample management, explicit multipart sample sync to compatible PocketTTS/OmniVoice/Chatterbox/XTTS services, and explicit local-provider voice discovery with a durable per-connector catalog available to NPC profile forms and revisioned default selection. |
...
| LLM, TTS, STT and API badges | Revisioned model slots, 22 TTS and 7 STT runtime adapters, provider-specific create/edit controls and defaults, isolated API-key management, testing, cloning, portable presets, active selection, and in-use deletion guards. |
...
| Global/configuration settings | Global Settings plus focused revisioned editors; rechat, boredom and greeting remain opt-in, bounded and active-session scoped. |
...
| ITT and Background Life/automatic diary systems | Explicitly excluded from the requested ALMSIVI scope. Manual narrator/diary/summary records remain available. |
