...
| Dialogue | Solo/group turns, speaker/addressee/audience, ordered deltas/final, delivery result, playback-gated rechat continuation with opt-in policy-checked actions, and optional bounded player mood cues projected into prompts/history without rewriting authored text. |
...
| Rechat | Included only as a depth-bounded continuation of a player-started conversation after final playback; an optional bounded client state snapshot filters unavailable participants and the selected responder's effective Global/Core Profile/NPC enablement is rechecked. Actions remain off by default and use the normal negotiated catalog and policy when enabled. |
| Automatic dialogue | Global controls enable game-scheduled greetings, boredom remarks, and combat barks through the existing bounded turn lane. ITT, Background Life, and AI Quest surfaces remain removed. |
...
| ITT, Background Life, AI Quests, and automatic diary systems | Still excluded. Automatic greetings, boredom remarks, and combat barks are active through the game-owned idle scheduler; playback-driven rechat, authored narrator/diary/summary records, explicitly requested one-shot diary generation, and player-triggered STT remain available. |
