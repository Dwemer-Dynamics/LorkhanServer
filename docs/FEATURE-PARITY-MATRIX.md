...
| Memory | Recent/middle/long memory, deterministic retrieval, source provenance, revisioned rebuild/edit/delete, and delivery-gated recent-memory derivation. Confirmed `played` dialogue queues one idempotent memory job; failed, expired, interrupted, stale, and partial output cannot satisfy the eligibility guard. Middle/long consolidation remains part of the active S4 checkpoint. |
...
| Timer autonomy, boredom, greetings, combat barks, ITT and Background Life/automatic diary systems | Explicitly excluded from the requested ALMSIVI scope. Playback-driven rechat, manual narrator/diary/summary records, and player-triggered STT remain available. |
