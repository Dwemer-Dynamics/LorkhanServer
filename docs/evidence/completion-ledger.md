...
Recorded baseline: 2026-07-19. Updated local WSL/browser acceptance: 2026-08-03 at clean server commit `40ebe0e59be36c13d1a8e115f4432c2fd4d9097c`; see `local-deployment-2026-08-03.md`. `AUTOMATED` means a repeatable local no-game check passed. `PLANNED` means required implementation/proof is not complete. `EXTERNAL-DEFERRED` requires an unavailable external environment or finalized predecessor evidence. `EXCLUDED` is a closed product boundary.
...
Local WSL deployment, Apache, worker, health, PostgreSQL integration, browser-like HTTP flows, and desktop browser rendering are now proven for the commits recorded above. Production systemd/immutable-release operation, in-game action/playback success, mobile browser rendering, CI, packaging, publication, and release remain unproven. No local mock or browser check is promoted to game evidence.
