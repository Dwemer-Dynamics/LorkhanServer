    ...
    <div class="roleplay-log-page<?= $responses?'':' book-log-page' ?><?= $books?' observed-books-page':'' ?>" data-log-page>
        ...
        <?php foreach($state['rows'] as $row): $id='log-entry-'.(int)$row['narrative_id']; ?>
            <dialog id="<?= $id ?>" class="log-content-modal<?= $responses?' response-prompt-viewer':($books?' book-content-viewer':'') ?>" aria-labelledby="<?= $id ?>-title">
                <header><h2 id="<?= $id ?>-title"><?= ($responses||$books)?'📜 Prompt Viewer':lorkhan_ui_h($row['title']) ?></h2><div><span role="status" data-log-status></span><button type="button" class="roleplay-button" data-log-copy><?= ($responses||$books)?'📋 Copy':'Copy' ?></button><button type="button" class="roleplay-button" data-log-close aria-label="Close reader"><?= ($responses||$books)?'×':'✕' ?></button></div></header>
                <div class="log-modal-body" data-log-copy-text>
                    <?php if($responses): ?>
                        <?php if(($row['prompt_label']??'')!==''||($row['prompt_driver']??'')!==''||($row['prompt_model']??'')!==''): ?><div class="prompt-metadata"><?php foreach(['prompt_label','prompt_driver','prompt_model'] as $metaKey): if(($row[$metaKey]??'')==='')continue; ?><span class="prompt-meta<?= ' prompt-meta-'.substr($metaKey,7) ?>"><?= lorkhan_ui_h($row[$metaKey]) ?></span><?php endforeach; ?></div><?php endif; ?>
                        ...
                        <?php if($row['prompt_messages']===[]): ?><pre class="prompt-raw">Prompt payload is empty for this row.</pre><?php endif; ?>
