    ...
    <div class="roleplay-log-page<?= $responses?'':' book-log-page' ?><?= $books?' observed-books-page':'' ?>" data-log-page>
        ...
        <?php foreach($state['rows'] as $row): $id='log-entry-'.(int)$row['narrative_id']; ?>
            <dialog id="<?= $id ?>" class="log-content-modal<?= $responses?' response-prompt-viewer':($books?' book-content-viewer':'') ?>" aria-labelledby="<?= $id ?>-title">
                <header><h2 id="<?= $id ?>-title"><?= $responses?'📜 Prompt Viewer':lorkhan_ui_h($row['title']) ?></h2><div><span role="status" data-log-status></span><button type="button" class="roleplay-button" data-log-copy><?= $responses?'📋 Copy':'Copy' ?></button><button type="button" class="roleplay-button" data-log-close aria-label="Close reader"><?= $responses?'×':'✕' ?></button></div></header>
