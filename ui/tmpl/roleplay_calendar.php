...
function lorkhan_roleplay_calendar(array $state, callable $link, string $tab): void
{
    ...
    <?php else: ?>
        ...
        for($cell=0;$cell<$cells;$cell++): if($cell%7===0)echo '<tr>'; $number=$cell-$offset+1;
            ...
            ?><td class="<?= $count>0?'has-event':'' ?><?= $date===$selectedDate?' selected-date':'' ?><?= !$game&&$date===gmdate('Y-m-d')?' current-date':'' ?>"><?php if($count>0): ?><a href="<?= lorkhan_ui_h($link([$game?'game_date':'date'=>$date,'reader_page'=>1]).($tab==='adventure'?'#adventure-events':'#event-table')) ?>" data-event-count="<?= $count ?>" aria-label="<?= lorkhan_ui_h($date) ?>: <?= $count ?> entries"<?= $date===$selectedDate?' aria-current="date"':'' ?>><?= $number ?></a><?php else: ?><span><?= $number ?></span><?php endif; ?></td><?php endif;
