...
function lorkhan_roleplay_reader_state(PDO $database, array $installationOptions, string $tab): array
{
    ...
    $state['total'] = (int) $statement->fetchColumn(); $pageSize = match($tab) { 'responselog'=>50, 'books'=>150, default=>20 }; $state['pages'] = max(1, (int) ceil($state['total'] / $pageSize));
    $state['page'] = max(1, min($state['pages'], (int) ($_GET['reader_page'] ?? 1)));
    $extra = match ($tab) { 'responselog' => ',n.turn_id,n.request_id,n.prompt_messages,n.input_kind,n.input_text,n.turn_seconds', 'adventure' => ',n.location,n.gamets,n.calendar_data', 'diaries' => ',n.calendar_data', 'books' => ',n.calendar_data,n.ts', default => '' };
...
function lorkhan_roleplay_reader(array $state, array $installationOptions, string $tab, string $webRoot, string $managementBasePath, string $csrf, array $preview): void
{
    ...
    <div class="roleplay-reader<?= $calendar?' calendar-log-page':'' ?>" data-reader data-preview-endpoint="<?= lorkhan_ui_h($managementBasePath.'/api/v1/tts-previews') ?>" data-installation="<?= lorkhan_ui_h($state['installation']) ?>" data-csrf="<?= lorkhan_ui_h($csrf) ?>" data-connector="<?= lorkhan_ui_h($preview['default_connector_id'] ?? '') ?>" data-voice="<?= lorkhan_ui_h($preview['default_voice'] ?? '') ?>" data-max-length="<?= SpeechPreviewCatalog::MAX_TEXT_LENGTH ?>">
        ...
        <?php if($tab==='diaries'): ?><td><?= lorkhan_ui_h($row['person']) ?></td><td><button type="button" class="log-content-link" data-calendar-open="entry-<?= lorkhan_ui_h($row['narrative_id']) ?>"><?= nl2br(lorkhan_ui_h(mb_substr($row['content'],0,700))) ?><?= mb_strlen($row['content'])>700?'…':'' ?></button></td><td><?= lorkhan_ui_h($row['game_date_label']) ?></td><td><?= lorkhan_ui_h(gmdate('d-m-Y H:i:s',strtotime($row['created_at']))) ?></td><td><button type="button" class="roleplay-button" data-calendar-open="entry-<?= lorkhan_ui_h($row['narrative_id']) ?>">Read / Edit</button></td>
