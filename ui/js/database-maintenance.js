...
(() => {
    ...
    const refresh = async () => {
        try {
            ...
            status.textContent = job ? labels[job.state] || 'Maintenance status unavailable.' : replay ? 'No migration replay requests.' : snapshot ? 'No snapshot save requests.' : restore ? 'No SQL restore requests.' : backup ? 'No SQL backup requests.' : 'No queued maintenance requests.';
