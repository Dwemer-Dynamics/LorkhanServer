...
(() => {
    ...
    function saveCard(card,explicit=false){
        return serial(async()=>{
            ...
            if(variable&&card.dataset.configured!=='false'&&card.querySelector('[data-custom-display-label]')){
                ...
                try{await post({action:'label',variable,display_label:displayValue},controller.signal);display.defaultValue=displayValue;}
                ...
                if(!value.trim()){status.textContent=display.value===displayValue?'Label saved. Saved key kept.':'Previous label saved. Your new label is still unsaved.';return;}
            }
            ...
            try{
                ...
                if(!variable){
                    ...
                    card.dataset.variable=result.variable;label.value=name.trim().toUpperCase();label.defaultValue=label.value;label.readOnly=false;label.dataset.customDisplayLabel='';label.maxLength=80;label.removeAttribute('pattern');
    ...
    window.addEventListener('beforeunload',event=>{
        if([...form.querySelectorAll('[data-key-card]')].some(card=>{
            const display=card.querySelector('[data-custom-display-label]');
            return !keyInput(card).disabled&&(keyInput(card).value!==''
                ||(!card.dataset.variable&&card.querySelector('[data-new-label]')?.value)
                ||(display&&display.value!==display.defaultValue));
        })){
