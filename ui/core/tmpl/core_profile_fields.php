...
$copyButton=static function(string$name,string$label)use($creatingProfile):void{
    ...
    $fields=['setting_quest_comments_enabled'=>'quest_comments.enabled','setting_quest_comments_chance_percent'=>'quest_comments.chance_percent','setting_bored_event_chance_percent'=>'bored_event.chance_percent','setting_response_core_lang'=>'response.core_lang','setting_response_max_words'=>'response.max_words','setting_behavior_rechat_max_depth'=>'behavior.rechat_max_depth',
...
foreach (['mid_term_enabled'=>'Middle Term Memory','short_term_enabled'=>'Short Term Memory'] as $field=>$label) {
    $toggleCard('setting_memory_'.$field,$field==='mid_term_enabled'?'&#x1F4C3;':'&#x1F5C2;&#xFE0F;',$label,'Include this memory tier in dialogue context.',($overrides['memory'][$field]??false)===true);
} ?></div></section><section class="profile-toggle-group"><h3 class="profile-toggle-group-title">Diary</h3><div class="profile-toggle-grid"><?php $toggleCard('setting_diary_automatic_enabled','&#x1F4D9;','Auto Diary','Generate diaries after sleeping.',$values['diary_automatic_enabled']);$toggleCard('setting_diary_automatic_wait_enabled','&#x23F3;','Auto Diary Wait','Also generate an automatic diary after waiting.',$values['diary_automatic_wait_enabled']);echo '<input type="hidden" name="diary_materialize_present" value="1">';$toggleCard('setting_diary_materialize_enabled','&#x1F4D5;','Physical Diary','Keep one readable diary book in the NPC’s inventory, updated from the latest five saved generated entries.',($overrides['diary']['materialize_enabled']??false)===true);$toggleCard('setting_diary_latest_entry_in_context','&#x1F4D6;','Include Latest Diary Entry',"Include the NPC's latest diary entry in response context.",($overrides['diary']['latest_entry_in_context']??false)===true); ?></div></section><section class="profile-toggle-group"><h3 class="profile-toggle-group-title">LLM</h3><div class="profile-toggle-grid"><?php $toggleCard('llm_randomizer_enabled','&#x1F3B2;','LLM Randomizer','Rotate among the four response models.',($routing['llm_randomizer_enabled']??false)===true);$toggleCard('llm_fallback_enabled','&#x1F504;','LLM Fallback','Retry with the fallback model.',($routing['llm_fallback_enabled']??false)===true); ?></div></section></div>
</div>

<div class="connector-card profile-connectors-card"><div class="connector-title">Connector Selection</div><div class="connector-subtitle">Choose response, voice, diary, and fallback connectors.</div><div class="connector-selection-grid"><section class="connector-group-card"><h3 class="connector-group-title">Response Connectors</h3><div class="connector-group-fields">
...
<div class="profile-feature-grid"><div class="provider-card">
    ...
    <div class="provider-body"><?php require_once __DIR__.'/../../tmpl/dynamic_profile_schedule.php'; lorkhan_dynamic_profile_schedule($evolution,'setting_profile_evolution_'); ?><div class="setting-row">
    ...
    <div class="provider-body profile-provider-body">
        <div class="setting-row">
            ...
            <div class="setting-control setting-control-wide">
                ...
                <?php foreach (['levelup','combat_end','sleep','lockpick'] as $event): ?>
...
</div></section><section class="profile-settings-group"><h2 class="profile-settings-heading">Context</h2><div class="provider-card profile-settings-group-card"><?php $numberField('setting_memory_recent_turn_limit','Context History Event Count','Maximum recent dialogue and world events in context. Set 0 to omit history.',$values['recent_turn_limit'],0,200);$numberField('setting_diary_context_turn_limit','Context History Diary Event Count','Maximum witnessed events in a diary. Set 0 to use regular Context History.',$values['diary_context_turn_limit'],0,400);$numberField('setting_profile_evolution_history_limit','Context History Dynamic Profile Event Count','Maximum witnessed events used by automatic NPC and Narrator profile updates. Set 0 to use regular Context History.',(int)($evolution['history_limit']??50),0,400); ?></div></section><section class="profile-settings-group profile-diary-settings"><h2 class="profile-settings-heading">Diary</h2><div class="provider-card profile-settings-group-card"><?php  ?><div class="setting-row profile-setting-stacked"><div><div class="setting-key"><span class="setting-icon" aria-hidden="true">&#x1F4D9;</span><label for="profile-diary-prompt">Diary Prompt</label> <?php $copyButton('setting_diary_prompt','Diary Prompt'); ?></div><div class="setting-desc">Instructions for generating diary entries for this profile.</div></div><div class="setting-control"><textarea id="profile-diary-prompt" name="setting_diary_prompt" rows="4" maxlength="8192" placeholder="Enter value"><?php echo lorkhan_ui_h($values['diary_prompt']); ?></textarea></div></div><?php $numberField('setting_diary_automatic_interval_seconds','Diary Cooldown','Cooldown in seconds between automatic diary entries for each NPC. Existing intervals above 1200 seconds are preserved.',$values['diary_automatic_interval_seconds'],10,max(1200,$values['diary_automatic_interval_seconds'])); ?></div></section>
