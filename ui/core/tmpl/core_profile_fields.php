...
$copyButton=static function(string$name,string$label)use($creatingProfile):void{
    ...
    $fields=['setting_response_lang_llm_xtts'=>'response.lang_llm_xtts','setting_response_core_lang'=>'response.core_lang','setting_response_max_words'=>'response.max_words','setting_behavior_rechat_max_depth'=>'behavior.rechat_max_depth',
...
$numberField=static function(string$name,string$label,string$description,int$value,int$min,int$max)use($copyButton):void{
    $icon=str_contains($name,'diary')?'&#x1F4D9;':(str_contains($name,'rechat')?'&#x1F501;':(str_contains($name,'history')||str_contains($name,'recent_turn')?'&#x1F9E0;':'&#x2699;&#xFE0F;')); ?>
