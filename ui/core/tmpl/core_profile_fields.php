...
} ?></div></section><section class="profile-toggle-group"><h3 class="profile-toggle-group-title">Diary</h3><div class="profile-toggle-grid"><?php $toggleCard('setting_diary_automatic_enabled','&#x1F4D9;','Auto Diary','Generate diaries on the timer and after sleeping.',$values['diary_automatic_enabled']);$toggleCard('setting_diary_automatic_wait_enabled','&#x23F3;','Auto Diary Wait','Also generate an automatic diary after waiting.',$values['diary_automatic_wait_enabled']);echo '<input type="hidden" name="diary_materialize_present" value="1">';$toggleCard('setting_diary_materialize_enabled','&#x1F4D5;','Physical Diary','Keep one readable diary book in the NPC’s inventory, updated from the latest five saved generated entries.',($overrides['diary']['materialize_enabled']??false)===true);$toggleCard('setting_diary_latest_entry_in_context','&#x1F4D6;','Include Latest Diary Entry',"Include the NPC's latest diary entry in response context.",($overrides['diary']['latest_entry_in_context']??false)===true); ?></div></section><section class="profile-toggle-group"><h3 class="profile-toggle-group-title">LLM</h3><div class="profile-toggle-grid"><?php $toggleCard('llm_randomizer_enabled','&#x1F3B2;','LLM Randomizer','Rotate among the four response models.',($routing['llm_randomizer_enabled']??false)===true);$toggleCard('llm_fallback_enabled','&#x1F504;','LLM Fallback','Retry with the fallback model.',($routing['llm_fallback_enabled']??false)===true); ?></div></section></div>
</div>

<section class="profile-settings-group"><h2 class="profile-settings-heading">Dynamic Profile Schedule</h2><div class="provider-card profile-settings-group-card">
  <div class="setting-row"><div><div class="setting-key"><label for="setting_profile_evolution_interval_days">Update Interval (Game Days)</label></div><div class="setting-desc">Elapsed game days before another update. Requires enough delivered events and the real-time cooldown.</div></div><input id="setting_profile_evolution_interval_days" name="setting_profile_evolution_interval_days" type="number" min="0.041666666666666664" max="365" step="any" value="<?php echo lorkhan_ui_h((string)($evolution['interval_days']??1)); ?>"></div>
  <?php $numberField('setting_profile_evolution_min_events','Minimum Events','Relevant delivered events required for each update. Combat barks do not count.',(int)($evolution['min_events']??30),1,10000);
  $numberField('setting_profile_evolution_cooldown_minutes','Cooldown (Real Minutes)','Minimum real minutes between automatic update attempts.',(int)($evolution['cooldown_minutes']??5),1,1440); ?>
</div></section>
