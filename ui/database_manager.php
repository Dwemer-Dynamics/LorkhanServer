...
$automaticBackupStats=$database->query("SELECT count(*) AS count,COALESCE(sum(byte_count),0) AS bytes FROM backup_records WHERE scope->>'kind'='database_sql' AND scope->>'automatic'='true'")->fetch(PDO::FETCH_ASSOC);
$sqlPage=max(1,min(100000,(int)($_GET['sql_page']??1)));
$sqlBackups=$database->query("SELECT backup_id,byte_count,state,scope->>'automatic' AS automatic,to_char(created_at AT TIME ZONE 'UTC','YYYY-MM-DD HH24:MI:SS') AS created_utc FROM backup_records WHERE scope->>'kind'='database_sql' ORDER BY created_at DESC,backup_id DESC LIMIT 26 OFFSET ".(($sqlPage-1)*25))->fetchAll(PDO::FETCH_ASSOC);
