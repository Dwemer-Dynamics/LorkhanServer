...
<main class="oghma-runtime-page">
    <header class="runtime-header"><div><h1>Oghma Audit</h1><p>See which topics Oghma found in each conversation and what knowledge it gave the NPC.</p></div><span class="runtime-count"><?php echo lorkhan_ui_h($audit['total']); ?> records</span></header>
    <section class="runtime-metrics" aria-label="Oghma audit summary">
        <div class="metric-panel"><span class="metric"><?php echo lorkhan_ui_h($audit['total']); ?></span><span class="metric-label">Audited requests</span><span class="metric-note">matching these filters</span></div>
    ...
    <form class="runtime-filters" method="get">
        ...
        <label>Installation<select name="installation_id"><option value="">All installations</option><?php foreach($installations as$row): ?><option value="<?php echo lorkhan_ui_h($row['installation_id']); ?>"<?php echo $filters['installation_id']===$row['installation_id']?' selected':''; ?>><?php echo lorkhan_ui_h($row['display_name']); ?></option><?php endforeach; ?></select></label>
        <label>Search<input name="search" maxlength="100" value="<?php echo lorkhan_ui_h($filters['search']); ?>" placeholder="Query, NPC, topic or reason"></label>
        ...
        <label>Topic match<select name="extractor"><?php foreach(['all'=>'All']+$extractorLabels as$value=>$label): ?><option value="<?php echo lorkhan_ui_h($value); ?>"<?php echo $filters['extractor']===$value?' selected':''; ?>><?php echo lorkhan_ui_h($label); ?></option><?php endforeach; ?></select></label>
    ...
    <?php foreach($audit['rows']as$row): $reasons=is_array($row['reasons']??null)?$row['reasons']:[];$context=is_array($reasons['_context']??null)?$reasons['_context']:[];unset($reasons['_context']);$selected=is_array($row['selected_topics']??null)?$row['selected_topics']:[];$resultCount=(int)$row['result_count']; ?>
        <article class="trace-card">
            <header><div><h2><?php echo lorkhan_ui_h($extractorLabel((string)($context['extractor_status']??'legacy'))); ?> &middot; <?php echo lorkhan_ui_h($row['profile_name']??'Unbound profile'); ?></h2><p><?php echo lorkhan_ui_h($row['created_at']); ?></p></div><span class="trace-result <?php echo $resultCount>0?'matched':'unmatched'; ?>"><?php echo $resultCount>0?lorkhan_ui_h($resultCount).' article'.($resultCount===1?'':'s').' shared':'Nothing shared'; ?></span></header>
            <div class="trace-row"><strong>Conversation</strong><span><?php echo lorkhan_ui_h($row['query']); ?></span></div>
            <div class="trace-row"><strong>Topics found</strong><span><?php echo lorkhan_ui_h(implode(', ',(array)($context['extracted_topics']??[]))?:'None'); ?></span></div>
            <div class="trace-row"><strong>Knowledge shared</strong><?php if($selected!==[]): ?><div class="selected-topics"><?php foreach($selected as$topic): $decision=$reasons[$topic['id']??'']??[];$category=trim((string)($topic['category']??'')); ?><span><?php echo lorkhan_ui_h($topic['topic']??'Unknown'); ?><small><?php echo lorkhan_ui_h($accessLabel((string)(is_array($decision)?($decision['access_level']??''):''))); ?><?php if($category!==''): ?> &middot; <?php echo lorkhan_ui_h($category); ?><?php endif; ?></small></span><?php endforeach; ?></div><?php else: ?><span class="trace-none">Nothing shared with this NPC</span><?php endif; ?></div>
            <details class="trace-technical">
                <summary>Technical details</summary>
                <div class="signal-grid">
                    <div><strong>Algorithm</strong><span><?php echo lorkhan_ui_h($row['algorithm']); ?></span></div>
                    <div><strong>Extractor status</strong><span><?php echo lorkhan_ui_h($context['extractor_status']??'legacy'); ?></span></div>
                    <div><strong>Race injection</strong><span><?php echo lorkhan_ui_h(implode(', ',(array)($context['race_signals']??[]))?:'None'); ?></span></div>
                    <div><strong>Location injection</strong><span><?php echo lorkhan_ui_h(implode(', ',(array)($context['location_signals']??[]))?:'None'); ?></span></div>
                    ...
                    <div><strong>Denied topics</strong><span><?php echo lorkhan_ui_h(implode(', ',(array)($context['denied_topics']??[]))?:'None'); ?></span></div>
                </div>
                ...
                <?php foreach(['grounded_matches'=>'Grounded matches','grounded_rejections'=>'Grounded rejections','tag_decisions'=>'Tag decisions','suggested_topics'=>'Connector suggestions']as$key=>$label): $items=is_array($context[$key]??null)?$context[$key]:[]; ?>
                    <details><summary><?php echo lorkhan_ui_h($label); ?> (<?php echo count($items); ?>)</summary><pre><?php echo lorkhan_ui_h($prettyJson($items)); ?></pre></details>
                <?php endforeach; ?>
                </div>
                <details><summary>Full retrieval decisions (<?php echo count($reasons); ?>)</summary><div class="decision-table"><table><thead><tr><th>Topic</th><th>Signal</th><th>Source</th><th>Score</th><th>Access</th><th>Selected</th><th>Reason</th></tr></thead><tbody><?php foreach($reasons as$decision):if(!is_array($decision))continue; ?><tr><td><?php echo lorkhan_ui_h($decision['topic']??''); ?></td><td><?php echo lorkhan_ui_h($decision['signal']??''); ?></td><td><?php echo lorkhan_ui_h($decision['source']??''); ?></td><td><?php echo lorkhan_ui_h($decision['score']??''); ?></td><td><?php echo lorkhan_ui_h($decision['access_level']??''); ?></td><td><?php echo ($decision['selected']??false)?'Yes':'No'; ?></td><td><?php echo lorkhan_ui_h($decision['reason']??''); ?></td></tr><?php endforeach; ?></tbody></table></div></details>
            </details>
        </article>
    <?php endforeach; ?>
    </section>
    <nav class="runtime-pagination" aria-label="Oghma audit pages"><?php if($audit['page']>1): ?><a href="<?php echo lorkhan_ui_h($pageUrl($audit['page']-1)); ?>">Previous</a><?php endif; ?><span>Page <?php echo $audit['page']; ?> of <?php echo $audit['pages']; ?></span><?php if($audit['page']<$audit['pages']): ?><a href="<?php echo lorkhan_ui_h($pageUrl($audit['page']+1)); ?>">Next</a><?php endif; ?></nav>
</main>
