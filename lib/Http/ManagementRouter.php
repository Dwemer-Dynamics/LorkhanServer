...
final class ManagementRouter
{
    ...
    private function submit(string $domain,Request $r):Response
    {
        ...
        // Keep LLM form actions inside their Configuration iframe and reopen the saved connector.
        if(in_array($domain,['providers','provider-revise','provider-rollback','provider-delete','provider-clone','provider-import'],true)
            &&($v['embed']??null)==='1'){
            ...
            if($domain!=='provider-delete'&&is_string($result['configuration_id']??null))$query['edit']=$result['configuration_id'];
            return$this->redirect($this->uiPath('providers').'?'.http_build_query($query));
        }
