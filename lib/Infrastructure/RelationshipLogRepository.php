...
SQL;
        ...
        $select="SELECT a.provider_attempt_id,a.state,a.error_code,a.started_at,a.finished_at,a.operation,
            ...
            a.metadata->'relationship_request' AS recorded_request,a.metadata->'relationship_proposal' AS recorded_proposal,
            a.metadata->'relationship_applied' AS recorded_applied,
...
SELECT t.input_text,t.target->>'display_name' AS npc,t.speaker->>'display_name' AS speaker,
    NOT EXISTS(SELECT 1 FROM dialogue_utterances u LEFT JOIN eventlog_metadata visible
        ON visible.projection_key='dialogue:'||u.dialogue_message_id::text AND visible.projection_kind='dialogue'
        WHERE u.turn_id=t.turn_id AND (u.delivery_state<>'played' OR visible.projection_key IS NULL OR visible.suppressed_at IS NOT NULL))
    AND (SELECT count(*)=max(utterance_count) FROM dialogue_utterances WHERE turn_id=t.turn_id) AS complete_context,
...
SQL);
        foreach($rows as &$row){
            ...
            $parts=[];$bytes=0;$available=0;$truncated=false;
            ...
            while($exchange=$context->fetch()){
                ...
                if(filter_var($exchange['complete_context'],FILTER_VALIDATE_BOOL))++$available;
                $part=($exchange['speaker']??'Speaker').': '.$exchange['input_text']."\n".($exchange['npc']??'NPC').': '.$exchange['reply'];
                if($bytes+strlen($part)>65536){if(!$truncated)$parts[]='[Further retained context omitted from this reader.]';$truncated=true;continue;}
                $parts[]=$part;$bytes+=strlen($part);
            }
            $row['context']=implode("\n\n",$parts);
            $row['context_note']='Retained source conversation; the exact provider prompt was not recorded.';
            $row['changes']=is_string($row['recorded_applied'])?json_decode($row['recorded_applied'],true):null;
            $row['request']='';$row['proposal']='';
            $request=is_string($row['recorded_request'])?json_decode($row['recorded_request'],true):null;
            // A frozen request may contain every source exchange: suppress it if even one
            // source/utterance has since been hidden or removed, rather than resurrecting text.
            if(is_array($request)&&$ids!==[]&&$available===count($ids)){
                $messages=$request['messages']??[];
                $users=array_filter(is_array($messages)?$messages:[],static fn(mixed $message):bool=>is_array($message)&&($message['role']??null)==='user'&&is_string($message['content']??null));
                $row['context']=implode("\n\n",array_column($users,'content'));
                $row['context_note']=($request['exact']??false)?'Recorded provider request (user message).':'Recorded typed provider input (mock/test provider).';
                $row['request']=json_encode($messages,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
                $row['proposal']=is_string($row['recorded_proposal'])?json_encode(json_decode($row['recorded_proposal'],true),JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE):'';
            }elseif(is_array($request)){
                $row['context']='';$row['context_note']='Recorded request hidden because source context is no longer available.';
            }
            unset($row['source_ids'],$row['source_event_id'],$row['recorded_request'],$row['recorded_proposal'],$row['recorded_applied']);
        }unset($row);
